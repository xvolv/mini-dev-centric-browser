  ...
  const renderTextSegment = (content, keyBase) => {
    ...
    const readList = (isOrdered) => {
      ...
      blocks.push(
        isOrdered ? (
          <ol
            className="ai-assistant__list"
            key={`${keyBase}-ol-${blocks.length}`}
          >
            {items}
          </ol>
        ) : (
  ...
  const handleSend = async () => {
    ...
    if (!settings.apiKey) {
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          text: "Add your Groq API key in Settings to enable AI.",
        },
      ]);
      return;
    }
    ...
    try {
      ...
      if (includeHtml && activeTabHtml) {
        ...
        const trimmedHtml =
          activeTabHtml.length > maxChars
            ? activeTabHtml.slice(0, maxChars)
            : activeTabHtml;
        const truncatedNote =
          activeTabHtml.length > maxChars
      ...
      const payload = {
        ...
        messages: buildMessages([
          ...messages,
          { role: "user", text: questionWithContext },
        ]),
      };
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", text: res.content || "(empty response)" },
      ]);
    } catch (error) {
      setMessages((prev) => [
        ...prev,
  ...
  return (
    <div className="tool-panel">
      ...
      >
        ...
        <span>
          Text size:{" "}
          {activeTabHtml ? `${activeTabHtml.length} chars` : "0 chars"}
        </span>
        <span>
          Captured:{" "}
          {activeTabHtmlUpdatedAt
            ? new Date(activeTabHtmlUpdatedAt).toLocaleTimeString("en-US", {
                hour12: false,
              })
            : "never"}
        </span>
