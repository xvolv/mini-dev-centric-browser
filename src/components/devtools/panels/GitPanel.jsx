...
export default function GitPanel() {
  ...
  const pathSep = () => (window.navigator.platform.toLowerCase().includes('win') ? '\\' : '/');
  ...
  const handleClone = async (repo) => {
    ...
    try {
      ...
      const res = await window.electronAPI?.githubClone?.(repo.clone_url, target);
    ...
    <div className="tool-panel">
      <div className="git-panel__status">
        <div className="git-panel__repo">
          ...
          <span className="git-panel__repo-path">
            {auth.authenticated ? `Signed in as ${auth.user?.login}` : 'Not signed in'}
          </span>
          <div className="git-panel__repo-actions">
            {!auth.authenticated ? (
              <button className="git-panel__repo-btn" onClick={startDeviceFlow}>Sign in with GitHub</button>
            ) : (
              <button className="git-panel__repo-btn" onClick={handleLogout}>Sign out</button>
            )}
            {auth.authenticated && (
              <button className="git-panel__repo-btn" onClick={() => loadRepos(1)}>
                Load Repos
              </button>
            )}
          </div>
        ...
        {deviceFlow && (
          <div className="git-panel__device">
            <div className="git-panel__device-row">1) Open: {deviceFlow.verification_uri}</div>
            <div className="git-panel__device-row">2) Enter code: <strong>{deviceFlow.user_code}</strong></div>
            <div className="git-panel__device-actions">
              <button className="git-panel__repo-btn" onClick={openVerification}>Open Login</button>
            </div>
      ...
      {auth.authenticated && repos.length > 0 && (
        <div className="git-panel__repos">
          <div className="git-panel__repos-header">
            ...
            />
            <button className="git-panel__repo-btn" onClick={chooseCloneDir}>Choose Storage</button>
          </div>
          {cloneDir && <div className="git-panel__repo-path">Storage: {cloneDir}</div>}
            ...
            {filteredRepos.map((repo) => (
              <div key={repo.id} className="git-panel__repo-card">
                <div>
                  ...
                  <div className="git-panel__repo-meta">{repo.private ? 'Private' : 'Public'} • {repo.default_branch}</div>
                </div>
                <button
      ...
      <div className="git-panel__status">
        <div className="git-panel__repo">
          ...
          <span className="git-panel__repo-path">{repoPath || 'Not selected'}</span>
          <div className="git-panel__repo-actions">
            <button className="git-panel__repo-btn" onClick={handleSelectRepo}>Select</button>
            <button className="git-panel__repo-btn" onClick={() => loadRepoData(repoPath)} disabled={!repoPath || loading}>
              Refresh
            </button>
          </div>
        </div>
        ...
        <div className="git-panel__branch">
          ...
          {branch || 'No branch'}
          <span className="tool-panel__badge tool-panel__badge--info">{statusBadge}</span>
        </div>
      </div>
      <div className="tool-panel__header">
        <span className="tool-panel__title">Changed Files</span>
        <span className="tool-panel__badge tool-panel__badge--warn">{changedCount}</span>
      </div>
      <ul className="git-panel__file-list">
        ...
        {files.length === 0 && (
          <li className="git-panel__file git-panel__file--empty">No changes detected.</li>
        )}
      </ul>
